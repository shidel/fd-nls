...
#  Codepage: 852
#  Author:   Dejan ætrbac (strbac@phreaker.net)
#
...
Loça komanda ili naziv fajla - "%s".
.

:TEXT_ERROR_OUT_OF_MEMORY
Greçka nedostatka memorije.
.
...
Promenljiva okru§enja '%s' nije pronaÐena.
.
...
Linija je preduga. Komanda ne mo§e biti izvrçena.
.

:TEXT_ERROR_HISTORY_SIZE#1
Neva§e†a veliŸina istorije - '%s'
.
...
MCB lanac je oçte†en, ili MS-DOS nekompatibilan sistem.
.

:TEXT_ERROR_UNDEFINED_ERROR
Nedefinisana greçka %d.
.

:TEXT_ERROR_REGION_WARNING
Neva§e†e memorijsko podruŸje %d - ignorisano.
.
...
Neva§e†i ili ne postoje†i uŸitno-upisni ureÐaj '%s'.
.
...
PrekidaŸ /L nije implementiran.
.

:TEXT_ERROR_U_NOTIMPLEMENTED
PrekidaŸ /U nije implementiran.
.

:TEXT_ERROR_WRITING_DEST
Greçka pri upisu na odrediçte.
.
...
Nemogu†nost Ÿitanja iz fajla: '%s'
.
...
Neuspeh pri uŸitavanju poruka u memoriju.
.

:TEXT_MSG_NOTIMPLEMENTED
PrekidaŸ /MSG nije implementiran.
.
...
Oçte†ena komandna linija. Ovo je interna greçka i povezana je sa
sistemom u kome radi COMMAND.COM. Molimo vas prijavite ovu greçku.
.
...
Ovo znaŸi ukljuŸujuŸi slovo drajva poŸinju†i obrnutom kosom crtom (\\).
Na primer: C:\\
...
su izgubljene. Molimo vas pogledajte gore izjavljene poruke o greçkama
za mogu†i razlog nastanka ovog problema.
.

:TEXT_ERROR_SAVE_SESSION
Trenutne informacije nemogu biti oŸuvane za vreme poziva programa.
Molimo vas pogledajte gore izjavljene poruke o greçkama za mogu†i razlog
...
Swapping nije uspeo: Aliasi zauzimaju previçe memorije.
.
...
Manipulator kritiŸnih greçaka ili kontekst modul ne mo§e biti uŸitan.
.

:TEXT_ERROR_CONTEXT_OUT_OF_MEMORY#1
Nedostatak memorije za kontekst.
Ako ova greçka nastavi da se javlja, razmotrite pove†anje internog
bafera, kao istorije, steka direktorijuma itd.
.

:TEXT_ERROR_CONTEXT_LENGTH#1
VeliŸina konteksta prelazi maksimalni limit za %lu bajta.
Promena veliŸine konteksta na %u bajta.
.

:TEXT_ERROR_CONTEXT_ADD_STATUS#1
Neuspeh pri dodavanju statusnih informacija u kontekst. Ova greçka
mo§e znaŸiti oçte†enje memorije ili nepravilno utvrÐena minimalna
veliŸina konteksta.
.
...
VeliŸina istorije je %d bajta.
.

:TEXT_MSG_DOSKEY
DOSKEY je ve† omogu†en interno u komandnoj ljusci.
Pokrenite DOSKEY /? za viçe informacija o implementaciji.
.
...
(obiŸno navoÐenjem prekidaŸa "/P").\r
Morate restartovati sistem ili, ukoliko ova ljuska radi u\r
viçeprocesnom okru§enju, prekinuti ovaj proces.\r
.
...
:TEXT_CMDHELP_BREAK
Postavlja ili uklanja proçirenu CTRL+C proveru.
...
Poziva jedan batch program iz drugog.
  ...
  /S ukljuŸuje dok /N iskljuŸuje swapping komandne ljuske.
  /Y omogu†ava mod pra†enja prilikom izvrçenja komande.
.
...
:TEXT_CMDHELP_CLS
Briçe ekran i postavlja standardne atribute boja.
...
Pokre†e novu instancu komandne ljuske.
  ...
  COMMAND [[drajv:]putanja] [ureÐaj] [/E:nnnnn] [/P] [/MSG]
           [/LOW] [/Y [/[C|K] komanda]]

  [drajv:]putanja  Navodi direktorijum koji sadr§i COMMAND.COM.
  ureÐaj           Navodi ureÐaj za komandni ulaz/izlaz.
  /E:nnnnn         Postavlja poŸetnu veliŸinu okru§enja na nnnnn bajta.
                   (nnnnn treba biti broj izmeÐu 256 i 32,768).
  /P               Postavlja novu komandnu ljusku za stalnu (nema izlaza).
  /LOW             Primorava komandnu ljusku da dr§i svoje stalne podatke
                   u niskoj memoriji.
  /MSG             UŸitava sve sistemske poruke u memoriju (zahteva i /P).
  /Y               KoraŸa kroz batch program naveden sa /C ili /K.
  /C komanda       Izvrçava navedenu komandu i vra†a se u trenutnu ljusku.
  /K komanda       Izvrçava navedenu komandu i nastavlja rad.
.

:TEXT_CMDHELP_COPY
Kopira jedan ili viçe fajlova na drugu lokaciju.

  COPY [/A | /B] izvor [/A | /B] [+ izvor [/A | /B] [+ ...]]
        [odrediçte [/A | /B]] [/V] [/Y | /-Y]

  izvor        Navodi fajl ili fajlove koje treba prekopirati.
  /A           Nagoveçta ASCII tekstualni fajl.
  /B           Nagoveçta binarni fajl.
  odrediçte    Navodi direktorijum i/ili naziv fajla za novi(e) fajl(ove).
  /V           Proverava korektnost upisa novih fajlova.
  /Y           Priguçava upite za pisanje preko ve† postoje†ih fajlova u
               odrediçtu, odgovaraju†i potvrdno.
  /-Y          Poziva upit za potvrdu pisanja preko ve† postoje†ih fajlova
               u odrediçtu.

 PrekidaŸ /Y mo§e biti naveden u COPYCMD promenljivi okru§enja.
 Ovo mo§e biti poniçteno sa /-Y na komandnoj liniji.

 Za spajanje fajlova, navedite jedan fajl za odrediçte, ali viçe fajlova za
 izvor (koriscenjem d§oker znakova ili fajl1+fajl2+fajl3 formatom).
.

:TEXT_CMDHELP_CTTY
Menja terminalni ureÐaj koriç†en za kontrolu sistema.

  CTTY ureÐaj

  ureÐaj    Terminalni ureÐaj koji §elite koristiti, kao npr. COM1.
.
...
:TEXT_CMDHELP_DEL#2
Briçe jedan ili viçe fajlova.

  DEL [drajv:][putanja]fajl [/P] [/V]
  ERASE [drajv:][putanja]fajl [/P] [/V]

  [drajv:][putanja]fajl   Navodi fajl(ove) za brisanje. Navedite viçe
                          fajlova koriste†i se d§oker znacima.
  /P    Poziva upit za potvrdu pre brisanja svakog fajla pojedinaŸno.
...
Prikazuje listu fajlova i pod-direktorijuma u direktorijumu.
  ...
  [drajv:][putanja][fajl]
           Navodi drajv, direktorijum, i/ili fajlove za listanje.
            (Mo§e biti slo§ena ili viçestruka specififikacija fajlova)
  /A       Prikazuje fajlove sa navedenim atributima. (Podraz. /ADHSRA)
  atributi  D  Direktorijumi             R  Read-only (samo za Ÿitanje)
            H  Hidden (sakriveni)        A  Spremni za arhiviranje
            S  Sistemski fajlovi         -  Prefiks negacije
  /O       Prikazuje fajlove sortirane na navedeni naŸin. (Podraz. /ONG)
  naŸin     N  Po nazivu                 S  Po veliŸini
            E  Po ekstenziji             D  Po datumu i vremenu
            G  Grupiçe direktorijume     -  Prefiks inverznog redosleda
            U  Nesortirano
  /P       Pauzira posle svakog punog ekrana informacija.
  /W       Koristi format çirokog listanja.
 ...
 PrekidaŸi mogu biti navedeni u DIRCMD promenljivi okru§enja. Poniçtite
 ve† navedene prekidaŸe postavljanjem prefiksa "-". Na primer: /-W.
.

:TEXT_CMDHELP_DOSKEY#1
DOSKEY je omogu†en interno u komandnoj ljusci.

 ÚÄÄÄÄÄÄÄÄÄÄÄÄÂÄÄÄÄÄÄÄÄÄÄ¿
 ³ Taster     ³ Funkcija ³
 ÃÄÄÄÄÄÄÄÄÄÄÄÄÅÄÄÄÄÄÄÄÄÄÄÁÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄ¿
 ...
 ³ Home/End   ³ PoŸetak/kraj linije.                                  ³
 ³ Insert     ³ Promena izmeÐu moda umetanja i pisanja preko.         ³
 ³ Tab        ³ Dopunjavanje trenutne reŸi kao naziv fajla. Dvostruki ³
...
:TEXT_CMDHELP_EXIT
Napuçta trenutno aktivnu komandnu ljusku.
...
Pokre†e navedenu komandu za svaki fajl iz navedenog skupa fajlova.
  ...
  (skup)        Navodi skup od jednog ili viçe fajlova. D§oker znaci su
                dozvoljeni.
  komanda       Navodi komandu za izvrçenje nad svakim fajlom iz skupa.
...
:TEXT_CMDHELP_GOTO
Upu†uje komandnu ljusku na liniju odreÐenu labelom u batch programu.

  GOTO labela

  labela   Navodi labelu definisanu u trenutnom batch programu.

 Labelu piçete samu na liniji, poŸinju†i dvotaŸkom.
.
...
:TEXT_CMDHELP_IF
Vrçi uslovno procesiranje u batch programima.

  IF [NOT] ERRORLEVEL broj komanda
  IF [NOT] string1==string2 komanda
  IF [NOT] EXIST fajl komanda

  NOT               Navodi da komandna ljuska treba da izvrçi komandu samo
                    ukoliko je uslov netaŸan. Negacija uslova.
  ERRORLEVEL broj   Vra†a TA¬NO ukoliko je prethodno pokrenuti program
                    vratio izlazni kod jednak ili ve†i od navedenog broja.
  komanda           Navodi komandu koja se treba izvrçiti ukoliko je uslov
                    zadovoljen.
...
:TEXT_CMDHELP_LH
UŸitava program u gornje memorijsko podruŸje.

  LOADHIGH [drajv:][putanja]fajl [parametri]
  LOADHIGH [/L:podruŸje1[,min_vel1][;podruŸje2[,min_vel2]...] [/S]]
           [drajv:][putanja]fajl [parametri]

  /L:podruŸje1[,min_vel1][;podruŸje2[,min_vel2]...
             Navodi podruŸje(a) memorije u koje treba uŸitati program.
             - podruŸje1 navodi broj prvog memorijskog podruŸja.
             - min_vel1 navodi minimalnu veliŸinu, ako postoji, za podruŸje1.
             - podruŸje2 navodi broj drugog memorijskog podruŸja.
             - min_vel2 navodi minimalnu veliŸinu, ako postoji, za podruŸje2.
             Mo§ete navesti koliko god §elite memorijskih podruŸja.

  /S         Smanjuje UMB do svoje minimalne veliŸine dok se program uŸitava.

  [drajv:][putanja]fajl
             Navodi lokaciju i naziv programa koji treba uŸitati.

  parametri  Parametri programa koji se uŸitava.
.

:TEXT_CMDHELP_LOADFIX
UŸitava program iznad prvih 64K memorije i pokre†e program.

  LOADFIX [drajv:][putanja]fajl

 Upotrebite LOADFIX za uŸitavanje programa ukoliko ste dobili poruku
 "Packed file corrupt" pri pokuçaju uŸitavanja u nisku memoriju.
.
...
:TEXT_CMDHELP_PATH
Prikazuje ili postavlja putanju za tra§enje izvrçnih fajlova.

  PATH [[drajv:]putanja[;...]]
  PATH ;

 Pokrenite "PATH ;" za brisanje svih putanja i usmerenje komandne
 ljuske na tra§enje izvrçnih fajlova samo u trenutnom direktorijumu.
...
Menja odziv komandnog prompta.
 ...
 Prompt mo§e biti sastavljen od znakova i slede†ih specijalnih kodova:
      ...
      $L   < (znak za manje-od)
      $B   | (znak za cev)
      $H   Backspace (briçe prethodni znak)
...
Skida direktorijum sa vrha steka direktorijuma, i postavlja ga za radni.
  ...
  Opcija '*' briçe sadr§aj steka direktorijuma.
.
...
Menja naziv fajlu ili direktorijumu.
 ...
 Primetite da ne mo§ete navesti novi drajv ili putanju za odrediçte.
...
Prikazuje, postavlja, ili uklanja promenljive okru§enja.
 ...
 Pokrenite SET bez parametara za prikaz trenutnih promenljivi okru§enja.
  ...
  /C    Prisiljava zadr§avanje navedene veliŸine slova promenljive.
  /P    Poziva upit sa stringom i dodeljuje korisniŸki unos promenljivoj.

 Promenljiva se standardno menja u velika slova, ukoliko se ve† ne
 nalazi u okru§enju, inaŸe se navedena veliŸina slova zadr§ava.
.
...
Menja poziciju zamenljivih parametara u batch fajlu.
 ...
 DOWN pomera prozor argumenata prema poŸetku (%0). U ostalim sluŸajevima
...
Prikazuje ili postavlja sistemsko vreme.
 ...
 upit za novu. Pritisnite ENTER da saŸuvate isto vreme.
.
...
Prikazuje listu komandi i osobina dostupnih u ljusci.
 ...
 izvrçiti, kao da je mod pra†enja aktivan.
.

:TEXT_CMDHELP_WHICH
Tra§i i prikazuje izvrçne fajlove za svaku navedenu komandu.
...
Odrediçte COPY ne sme sadr§ati plus ('+') znakove.
.
